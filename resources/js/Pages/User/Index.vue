...
const table = reactive({
    ...
    rows: [],
    totalRecordCount: 0,
    pageSize: localStorage.users_limit,
    page: localStorage.users_page,
    sortable: {
        order: localStorage.users_order ?? 'id',
        sort: localStorage.users_sort ?? 'desc',
    },
...
const doSearch = (offset, limit, order, sort, type = null) => {
    ...
    axios.get(url).then((response) => {
        ...
        localStorage.users_sort = sort;
        localStorage.users_type = type;
        localStorage.users_order = order;
        localStorage.users_offset = offset;
        localStorage.users_limit = limit;
        localStorage.users_page = page;
    });
};
...
watch(
    searchTerm,
    _.debounce(() => {
        if (searchTerm.value !== localStorage.users_searchTerm) {
            doSearch(
                0,
                localStorage.users_limit ?? 10,
                localStorage.users_order ?? 'id',
                localStorage.users_sort ?? 'desc',
                localStorage.users_type ?? null
            );
            localStorage.users_searchTerm = searchTerm.value;
        }
    }, 500)
);
...
const deleteRow = (id) => {
    if (confirm('Czy na pewno chcesz usunąć ten element?')) {
        router.delete(route('users.destroy', id), {
            preserveScroll: true,
            onSuccess: () => {
                doSearch(localStorage.users_offset ?? 0, localStorage.users_limit ?? 10, localStorage.users_order ?? 'id', localStorage.users_sort ?? 'desc', localStorage.users_type ?? null);
                toast.success({
                    component: Toast,
                    props: {
...
const deleteRowsWithArray = async () => {
    if (ids_array.value.length >= 1 && confirm('Czy na pewno chcesz usunąć zaznaczone elementy?')) {
        router.get(route('users.destroyArray', {ids: ids_array.value}), {}, {
            preserveScroll: true,
            onFinish: () => {
                doSearch(localStorage.users_offset ?? 0, localStorage.users_limit ?? 10, localStorage.users_order ?? 'id', localStorage.users_sort ?? 'desc', localStorage.users_type ?? null);
                toast.success({
...
onMounted(() => {
    if (localStorage.users_searchTerm) {
        searchTerm.value = localStorage.users_searchTerm;
    }
                    ...
                    <div class="mt-4">
                        <div class="mb-3 sm:flex sm:justify-between">
                            ...
                            <div class="flex flex-row">
                                <Link :href="route('users.create')" disabled>
                                    <div
